...
Application inventory presents one outbound endpoint or DNS question even when several Linux process threads produced it. The thread command remains visible on each retained raw occurrence and in the process-aware Runtime Groups linked from the item.
