...
Application inventory groups the same system call across Linux process threads. Open a retained raw occurrence when you need the originating process command. Runtime Groups remain process-aware, and process execution and termination identities are unchanged.
...
Application inventory presents one outbound endpoint even when several Linux process threads produced it. For DNS, the inventory presents a logical destination and combines A/AAAA questions plus confidently corroborated Kubernetes search expansions. Expand **DNS resolution variants** to inspect every exact question name, type, and observation count or to open its unchanged evidence history. Grouping changes presentation only: exact DNS identities remain the basis for retained evidence and policies. If the filtered evidence cannot establish an expansion confidently, the names remain separate.
