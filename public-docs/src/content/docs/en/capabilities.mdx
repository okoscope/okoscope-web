...
Application diagnostics are positive changes during the selected range, not lifetime totals. They include only losses and delivery outcomes assigned to the selected workload's authenticated Application stream, such as a full route queue, route rate limiting, or a delivery retry. Resets are marked separately for that Application route. Failures before workload attribution and other node-wide diagnostics remain in agent logs and internal operator telemetry; they are never projected into an Application. During an older-agent upgrade, scoped diagnostics are shown as unavailable rather than zero because absence of the scoped snapshot cannot prove that no loss occurred. Health history is retained for at least 25 hours, and each response is limited to 20 agents and at most 96 timeline intervals per agent.

![Compact Application agent health cards with workload-scoped diagnostics, a time axis, distinct heartbeat symbols, and capability details collapsed.](/documentation/application-health.en.png)
