...
Application inventory presents one outbound endpoint or exact DNS identity even when several Linux process threads produced it. DNS entries keep the observed question name and A or AAAA type visible in the main list and distribution, with their own evidence history, policy state, filters, and pagination. Kubernetes search-expanded questions therefore remain separate recorded identities instead of being folded into one inferred destination.
